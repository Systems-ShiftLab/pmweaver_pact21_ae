...
#ifdef WARN_SUYASH__
    #define warn__(msg, ...) \
    do {fprintf(stderr, RED "[WARN] " HYEL "%s" RST ":" HBLU "%d" RST ":" HCYN "%s()" RST       \
                " :: " msg "\n", __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__ );             \
    } while (0); 
#else
    #define warn__(msg, ...) \
...
#ifdef ERROR_SUYASH__
    #define error__(msg, ...) \
    do {fprintf(stderr, RED "[ ERR] " HYEL "%s" RST ":" HBLU "%d" RST ":" HCYN "%s()" RST       \
                " :: " msg "\n", __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__ );             \
        exit(EXIT_FAILURE);                                                                     \
    } while (0); 
#else
